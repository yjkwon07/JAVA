 ...
 * 글 상세창(viewArticle.jsp)에서 수정하기를 클릭해 글 정보를 표시하는 입력창들을 활성화
 * 글 정보와 이미지를 수정한 후 수정반영하기를 클릭해 컨트롤러에 /board/modArticle.do로 요청
 * 컨트롤러는 요청에 대해 upload()메소드를 이용하여 수정된 데이터를 Map에 저장하고 반환
		...
		try {
			...
			if (action == null) {
				...
				if (imageFileName != null && imageFileName.length() != 0) {
					...
					destDir.mkdirs();
					FileUtils.moveFileToDirectory(srcFile, destDir, true);
					// 전송된  orginallmageFileName을 이용해 기존의 파일을 삭제
					File oldFile = new File(ARTICLE_IMAGE_REPO + "\\" + articleNO + "\\" + originalFileName);
					oldFile.delete();
				}
				PrintWriter pw = response.getWriter();
				// 글 수정 후 location 객체의 href 속성을 잉요해 글 상세 화면을 나타낸다.
				pw.print("<script>" + "  alert('글을 수정했습니다.');" + " location.href='" + request.getContextPath()
						+ "/boardD/viewArticle.do?articleNO=" + articleNO + "';" + "</script>");
				return;
			}
